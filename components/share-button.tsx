...
export function ShareButton({ 
  ...
  const ShareMenuItem = ({ onClick, icon, label }: { onClick: () => void; icon: React.ReactNode; label: string }) => (
    <button 
      type="button"
      onClick={(e) => {
        e.preventDefault();
      ...
      className="w-full flex items-center gap-3 px-3 py-2.5 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg text-sm transition-colors cursor-pointer"
    >
      {icon}
      <span>{label}</span>
    </button>
      ...
      {mounted && showMenu && createPortal(
        ...
        >
          ...
          <ShareMenuItem 
            ...
            icon={copied ? <Check className="h-4 w-4 text-emerald-500" /> : <Copy className="h-4 w-4 text-gray-500" />}
