...
// GET: Fetch comments for a promo
...
export async function POST(request: Request) {
  try {
    ...
    const { promoId, userId, userName, userAvatar, content, rating } = body;

    if (!promoId || !userId || !content) {
    ...
    // Check rate limit (24 hours between comments on same promo)
    ...
    if (recentComment) {
      ...
      if (hoursSinceLastComment < 24) {
        ...
        return NextResponse.json({ 
          error: `您已在 24 小時內評論過此優惠，請 ${hoursRemaining} 小時後再試。` 
        }, { status: 429 });
      }
    }
    ...
    if (error) {
      console.error('[Promo Comments API] Insert error:', error);
      throw error;
    }

    return NextResponse.json({ success: true, comment: data });
  } catch (error: any) {
    console.error('[Promo Comments API] Error creating comment:', error);
    return NextResponse.json({ error: error.message }, { status: 500 });
  }
}
